...
/**
 * @FileName: CronMinutePageStructure
 * @Desctiption: cron 分页面
 * @Author: hlb
 * @Date: Created by 2021/2/8 9:38
 * @Modified Update By:
 */
public class CronMinutePageStructure extends CronPageStructure {

    public CronMinutePageStructure() {

    }

    public CronMinutePageStructure(DateUnitEnum unit, ExpressionBo expressionBo
...
// ------------------------------------------- 构造函数 get/set结束线--------------------------------
    ...
    public JPanel getTopForm(DateUnitEnum page) {
        ...
        JRadioButton wildcardButton = new JRadioButton("每分钟 允许的通配符[, - * /]", true);
        wildcardButton.addActionListener(e -> {
            getExpressionBo().setMinuteTextField("*");
        ...
        JLabel radioFormSecondRowLastLabel = new JLabel("分钟");
        ...
        for (int i = this.getExpressionBo().getCurrentUnitMinValue(); i <= this.getExpressionBo().getCurrentUnitMaxValue(); i++) {
            ...
            DesignCheckBoxActionListener actionListener = new DesignCheckBoxActionListener()
                    ...
                    .setTextField(getExpressionBo().getMinuteTextField());
